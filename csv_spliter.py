...
def get_arguments():
  # use parse commands to get line arguments
...
def is_valid_csv(parser, file_name, row_limit):
  ...
  for row in csv.reader(open(file_name)):
   ...
   if row_limit > row_count:
    parser.error("The 'row_count' of '{}' is > the number of rows in '{}'!"
...
def parse_file(arguments):
  ...
  with open(input_file, 'r') as input_csv:
    ...
    header = all_rows.pop(0)
		...
		# Split list of lists into chunks
		current_chunk = 1
		for i in range(0, len(all_rows), row_limit):
			chunk = all_rows[i:i + row_limit]
			
			current_output = os.path.join(output_path,
